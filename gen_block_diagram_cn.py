#!/usr/bin/env python3
"""Generate system block diagram for DDS Signal Generator (Chinese version)."""
...
ax.add_patch(pl_box)
ax.text(6, 4.9, 'PL（可编程逻辑）', fontsize=12, fontweight='bold',
...
ax.text(3, 3.95, '按键控制模块', fontsize=10, fontweight='bold',
        ha='center', va='center', color='purple')
ax.text(3, 3.55, '消抖 | 频率/波形控制', fontsize=8,
        ha='center', va='center', color='purple')
...
ax.text(9, 3.95, 'UART解析模块', fontsize=10, fontweight='bold',
        ha='center', va='center', color='brown')
ax.text(9, 3.55, '串口接收 | 指令解析', fontsize=8,
        ha='center', va='center', color='brown')
...
ax.text(6, 2.2, 'DDS核心模块', fontsize=11, fontweight='bold',
        ha='center', va='center', color='red')
ax.text(6, 1.8, '相位累加器 + 波形查找表 + 波形选择',
...
ax.text(0.3, 4.5, '输入信号：', fontsize=9, fontweight='bold', ha='right')
...
ax.text(11.7, 1.2, '输出信号：', fontsize=9, fontweight='bold', ha='left')
ax.text(11.7, 0.9, '8位DAC输出', fontsize=8, ha='left')

plt.tight_layout()
plt.savefig('/mnt/d/FPGAmoudle/--main/--main/system_block_diagram_cn.png',
            dpi=200, bbox_inches='tight', facecolor='white')
plt.close()
print("中文系统框图已保存为 system_block_diagram_cn.png")
